...
foreach ( (array)$group_wiki_page_ids_array as $key => $group_wiki_page_id ) {
	...
	if ( bp_wiki_can_view_wiki_page( $group_wiki_page_id ) ) {
		...
		// TODO: only show edit link if is group member and has edit privileges for this page. 
		echo ' - <a href="' . bp_wiki_get_group_page_url( $bp->groups->current_group->id, $group_wiki_page_id ) . '/edit">'.__('Edit', 'bp-wiki').'</a> </div>';
		echo '</div>';
	}
}
