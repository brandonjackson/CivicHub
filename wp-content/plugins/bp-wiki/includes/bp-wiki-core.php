			...
			foreach ( $bp->action_variables as $key => $bp_action_var ) {
				...
				if ( bp_wiki_can_view_wiki_page( $wiki_page->ID ) ) {
					...
					if ( $key == ( count( $bp->action_variables ) - 1 ) ) {
						...
						$output .= '<li class="bp-wiki-group-breadcrumbs"><a href="' . $group_url . $action_slug . '/' . bp_wiki_remove_group_id_from_page_slug( $wiki_page->post_name, $bp->groups->current_group_id ) . '">' . $wiki_page->post_title . '</a>&nbsp;&raquo;</li>';
